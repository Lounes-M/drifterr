...
| Web surface hardening | 🟡 on `main` | CSP, HSTS and frame protection on the site (it had none), plus rate limits — per-user and durable for checkout, and a deliberately *global* bucket for the marketing endpoints, because keying on IP would create the visitor identifier `/api/event` promises does not exist |
